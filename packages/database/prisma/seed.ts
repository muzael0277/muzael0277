...
if (process.env.NODE_ENV === 'production' && process.env.ALLOW_PRODUCTION_SEED !== 'yes') {
  console.error('Refusing to seed a production database. Set ALLOW_PRODUCTION_SEED=yes to override.');
  process.exit(1);
}
...
async function seedPlans() {
  const plans = [
    { key: 'FREE', name: uz('Bepul', 'Бесплатный'), monthlyPrice: 0, sortOrder: 0,
      features: { orders_per_month: 50, products: 30, employees: 2, branches: 1, advanced_analytics: false, ai_assistant: false, marketing_automation: false, custom_domain: false } },
    { key: 'START', name: uz('Boshlang‘ich', 'Стартовый'), monthlyPrice: 149000, sortOrder: 1,
      features: { orders_per_month: 500, products: 300, employees: 5, branches: 2, advanced_analytics: false, ai_assistant: false, marketing_automation: false, custom_domain: false } },
    { key: 'BUSINESS', name: uz('Biznes', 'Бизнес'), monthlyPrice: 349000, sortOrder: 2,
      features: { orders_per_month: 3000, products: 2000, employees: 20, branches: 5, advanced_analytics: true, ai_assistant: false, marketing_automation: true, custom_domain: false } },
    { key: 'PRO', name: uz('Pro', 'Про'), monthlyPrice: 749000, sortOrder: 3,
      features: { orders_per_month: null, products: null, employees: 100, branches: 20, advanced_analytics: true, ai_assistant: true, marketing_automation: true, custom_domain: true } },
    { key: 'ENTERPRISE', name: uz('Korxona', 'Корпоративный'), monthlyPrice: 0, sortOrder: 4, isPublic: false,
      features: { orders_per_month: null, products: null, employees: null, branches: null, advanced_analytics: true, ai_assistant: true, marketing_automation: true, custom_domain: true } },
  ];
...
async function seedTemplates() {
  for (const def of Object.values(TEMPLATE_DEFINITIONS)) {
    await prisma.businessTemplate.upsert({
      where: { key: def.key },
      update: {
        name: def.label, description: def.description, icon: def.icon,
      ...
      create: {
        key: def.key, name: def.label, description: def.description, icon: def.icon,
        modules: def.modules as unknown as Prisma.InputJsonValue,
        moduleConfig: def.moduleConfig as Prisma.InputJsonValue,
        miniAppTabs: def.miniAppTabs as unknown as Prisma.InputJsonValue,
        orderPipeline: def.orderPipeline as unknown as Prisma.InputJsonValue,
...
async function seedFeatureFlags() {
  const flags = [
    ...
    { key: 'MARKETING_AUTOMATION', description: 'Campaigns and automated journeys', enabled: false },
    { key: 'POS_BETA', description: 'Point of sale beta', enabled: false },
    { key: 'NEW_MINIAPP', description: 'Next-generation Mini App shell', enabled: false, rolloutPercent: 10 },
    { key: 'REALTIME_ORDERS', description: 'Live order board via websockets', enabled: true },
  ];
  for (const flag of flags) {
    await prisma.featureFlag.upsert({
      where: { key: flag.key }, update: { description: flag.description }, create: flag,
    });
  }
...
async function createTenant(spec: TenantSpec) {
  ...
  await prisma.tenantSettings.create({
    data: {
      ...
      deliverySettings: { enabled: modules.includes('DELIVERY'), flatFee: 15000, freeAbove: 200000, minOrderTotal: 50000, estimatedMinutes: 45 },
      bookingSettings: { slotStepMinutes: 15, minLeadTimeMinutes: 60, maxAdvanceDays: 30, autoConfirm: true, requirePrepayment: false, cancellationDeadlineMinutes: 120, reminderOffsetsMinutes: [1440, 120] },
      loyaltySettings: { enabled: true, type: 'CASHBACK', rate: spec.loyaltyRate, minOrderTotal: 0, maxRedeemPercent: 50, expiryDays: null },
    },
  });
  ...
  await prisma.subscription.create({
    data: {
      ...
      currentPeriodStart: daysAgo(15), currentPeriodEnd: daysAhead(15),
    },
  });
  ...
  await prisma.customerSegment.createMany({
    data: [
      { tenantId: tenant.id, key: 'new_customers', isSystem: true, name: uz('Yangi mijozlar', 'Новые клиенты'),
        filter: { all: [{ field: 'orderCount', op: 'lte', value: 1 }] } },
      { tenantId: tenant.id, key: 'repeat_customers', isSystem: true, name: uz('Qaytgan mijozlar', 'Повторные клиенты'),
        filter: { all: [{ field: 'orderCount', op: 'gte', value: 2 }] } },
      { tenantId: tenant.id, key: 'vip', isSystem: true, name: uz('VIP mijozlar', 'VIP-клиенты'),
        filter: { all: [{ field: 'totalSpent', op: 'gte', value: 1000000 }] } },
      { tenantId: tenant.id, key: 'inactive_30d', isSystem: true, name: uz('30 kun faol emas', 'Неактивны 30 дней'),
        filter: { all: [{ field: 'lastActivityAt', op: 'daysAgoGt', value: 30 }] } },
      { tenantId: tenant.id, key: 'birthday_today', isSystem: true, name: uz('Bugun tug‘ilgan kun', 'День рождения сегодня'),
...
export { prisma, createTenant, seedPlans, seedTemplates, seedFeatureFlags, uz, daysAgo, daysAhead, FULL_WEEK_HOURS, WORKDAY_HOURS, DEMO_PASSWORD, randomCode, formatOrderNumber };
...
async function seedAnorCafe() {
  ...
  const branches = await Promise.all([
    prisma.branch.create({ data: { tenantId, name: 'Chilonzor filiali', address: "Chilonzor tumani, Bunyodkor ko'chasi 12", phone: '998901234567', latitude: 41.2856, longitude: 69.2034, workingHours: FULL_WEEK_HOURS as Prisma.InputJsonValue, isDefault: true } }),
    prisma.branch.create({ data: { tenantId, name: 'Yunusobod filiali', address: "Yunusobod tumani, Amir Temur ko'chasi 108", phone: '998901234568', latitude: 41.3421, longitude: 69.2876, workingHours: FULL_WEEK_HOURS as Prisma.InputJsonValue } }),
  ]);
  ...
  const sizeGroup = await prisma.modifierGroup.create({
    data: {
      tenantId, name: uz('Hajmi', 'Размер'), minSelect: 1, maxSelect: 1, sortOrder: 0,
      options: { create: [
  ...
  const extrasGroup = await prisma.modifierGroup.create({
    data: {
      tenantId, name: uz('Qo‘shimchalar', 'Добавки'), minSelect: 0, maxSelect: 5, sortOrder: 1,
      options: { create: [
        { tenantId, name: uz('Qo‘shimcha pishloq', 'Дополнительный сыр'), price: 8000, sortOrder: 0 },
        { tenantId, name: uz('Qo‘shimcha go‘sht', 'Дополнительное мясо'), price: 15000, sortOrder: 1 },
  ...
  const menu: {
    name: { uz: string; ru?: string }; price: number; categoryIdx: number;
    featured?: boolean; groups?: string[]; oldPrice?: number;
  }[] = [
    { name: uz('Osh', 'Плов'), price: 35000, categoryIdx: 0, featured: true, groups: [extrasGroup.id] },
    ...
    { name: uz('Shashlik (qo‘y)', 'Шашлык (баранина)'), price: 42000, categoryIdx: 0, groups: [extrasGroup.id] },
    { name: uz('Burger', 'Бургер'), price: 45000, categoryIdx: 1, featured: true, groups: [sizeGroup.id, extrasGroup.id] },
    { name: uz('Pepperoni pizza', 'Пицца Пепперони'), price: 69000, categoryIdx: 1, featured: true, oldPrice: 79000, groups: [sizeGroup.id, extrasGroup.id] },
    { name: uz('Margarita pizza', 'Пицца Маргарита'), price: 59000, categoryIdx: 1, groups: [sizeGroup.id, extrasGroup.id] },
    ...
    { name: uz('Cappuccino', 'Капучино'), price: 25000, categoryIdx: 3, featured: true, groups: [sizeGroup.id] },
  ...
  await prisma.promoCode.createMany({
    data: [
      { tenantId, code: 'ANOR10', type: 'PERCENTAGE', value: 10, minOrderTotal: 100000, maxDiscount: 30000, usageLimit: 500, perCustomerLimit: 2, endsAt: daysAhead(45) },
      { tenantId, code: 'YANGI15', type: 'PERCENTAGE', value: 15, minOrderTotal: 80000, maxDiscount: 25000, perCustomerLimit: 1, endsAt: daysAhead(90) },
      { tenantId, code: 'OSH20000', type: 'FIXED', value: 20000, minOrderTotal: 150000, usageLimit: 200, endsAt: daysAhead(30) },
    ],
  });
  ...
  const customerSpecs = [
    ...
    { firstName: 'Malika', lastName: 'Yusupova', phone: '998904445566', tgId: 504444444n, language: 'ru' },
    ...
    { firstName: 'Zarina', lastName: 'Umarova', phone: '998908889900', tgId: 508888888n, language: 'ru' },
  ];
  ...
  await seedOrderHistory(tenantId, customers, products, branches.map((b) => b.id), 5);
...
async function seedOrderHistory(
  ...
  const statuses = ['COMPLETED', 'COMPLETED', 'COMPLETED', 'COMPLETED', 'DELIVERING', 'PREPARING', 'ACCEPTED', 'NEW', 'CANCELLED'] as const;
  ...
  for (let i = 0; i < 28; i++) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        createdAt,
        updatedAt: createdAt,
        acceptedAt: status !== 'NEW' && status !== 'CANCELLED' ? new Date(createdAt.getTime() + 5 * 60000) : null,
        ...
        items: {
          create: lines.map((l) => ({
            ...
            quantity: l.quantity,
            discount: 0,
            total: (l.unitPrice + l.modifiersPrice) * l.quantity,
          })),
        },
        statusHistory: {
    ...
    if (isPaid) {
      await prisma.payment.create({
        data: {
          tenantId, orderId: order.id, customerId: customer.id,
          provider: method.toLowerCase(), method, state: 'PAID', amount: total,
    ...
    if (loyaltyEarned > 0) {
      await prisma.$transaction(async (tx) => {
        ...
        await tx.loyaltyTransaction.create({
          data: {
            tenantId, accountId: account.id, orderId: order.id, type: 'EARN',
            amount: loyaltyEarned, balanceAfter,
            reason: `Buyurtma ${order.orderNumber} uchun keshbek`, createdAt,
          },
        });
    ...
    await prisma.customerTimelineEntry.create({
      data: {
        tenantId, customerId: customer.id, type: 'ORDER_CREATED',
...
async function seedBarberHouse() {
  ...
  const branch = await prisma.branch.create({
    data: {
      tenantId, name: 'Barber House — Mirobod', address: "Mirobod tumani, Shota Rustaveli ko'chasi 45",
      phone: '998911234567', latitude: 41.2995, longitude: 69.2401, isDefault: true,
      workingHours: WORKDAY_HOURS as Prisma.InputJsonValue,
    },
  });

  const categories = await Promise.all(
    [uz('Soch', 'Волосы'), uz('Soqol', 'Борода'), uz('Kompleks', 'Комплекс')]
      .map((name, i) => prisma.category.create({ data: { tenantId, name, sortOrder: i } })),
  );

  const services = await Promise.all([
    prisma.service.create({ data: { tenantId, categoryId: categories[0]!.id, name: uz('Soch olish', 'Стрижка'), price: 100000, durationMinutes: 45, bufferAfterMinutes: 10, sortOrder: 0, description: uz('Mashinka va qaychi bilan zamonaviy soch turmagi') } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[1]!.id, name: uz('Soqol olish', 'Бритьё бороды'), price: 60000, durationMinutes: 30, bufferAfterMinutes: 10, sortOrder: 1 } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[2]!.id, name: uz('Soch + soqol', 'Стрижка + борода'), price: 140000, durationMinutes: 75, bufferAfterMinutes: 15, sortOrder: 2 } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[0]!.id, name: uz('Bolalar sochi', 'Детская стрижка'), price: 70000, durationMinutes: 30, sortOrder: 3 } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[1]!.id, name: uz('Qirqish (royal shave)', 'Королевское бритьё'), price: 90000, durationMinutes: 45, sortOrder: 4 } }),
  ]);

  const masters = [
    { firstName: 'Jamshid', lastName: 'Qodirov', position: 'Bosh usta', serviceIdx: [0, 1, 2, 3, 4] },
  ...
  for (const [i, master] of masters.entries()) {
    const employee = await prisma.employee.create({
      data: {
        tenantId, branchId: branch.id, firstName: master.firstName, lastName: master.lastName,
        position: master.position, phone: `99891123456${i + 1}`, sortOrder: i,
        services: { create: master.serviceIdx.map((si) => ({ tenantId, serviceId: services[si]!.id })) },
      },
    });
    ...
    const resource = await prisma.bookingResource.create({
      data: {
        tenantId, branchId: branch.id, employeeId: employee.id,
        name: `${master.firstName} ${master.lastName}`, kind: 'EMPLOYEE',
  ...
  await prisma.resourceTimeOff.create({
    data: {
      tenantId, resourceId: resources[1]!.id,
      startsAt: daysAhead(3, 0), endsAt: daysAhead(4, 0), reason: 'Dam olish kuni',
    },
  });
  ...
  for (const [i, spec] of clientSpecs.entries()) {
    clients.push(await prisma.customer.create({
      data: {
        tenantId, telegramUserId: spec.tgId, telegramUsername: `${spec.firstName.toLowerCase()}_uz`,
        firstName: spec.firstName, lastName: spec.lastName, phone: spec.phone,
        source: 'TELEGRAM', lastActivityAt: daysAgo(i), createdAt: daysAgo(45 - i * 4),
        loyaltyAccount: { create: { tenantId, balance: 0 } },
      },
    }));
  }
...
async function seedBookingHistory(
  tenantId: string,
  clients: { id: string }[],
  services: { id: string; name: Prisma.JsonValue; price: number; durationMinutes: number; bufferAfterMinutes: number }[],
  ...
  const plan: { dayOffset: number; status: 'COMPLETED' | 'CONFIRMED' | 'PENDING' | 'CANCELLED' | 'NO_SHOW' }[] = [
    { dayOffset: -14, status: 'COMPLETED' }, { dayOffset: -12, status: 'COMPLETED' },
    { dayOffset: -10, status: 'COMPLETED' }, { dayOffset: -9, status: 'NO_SHOW' },
    { dayOffset: -7, status: 'COMPLETED' }, { dayOffset: -5, status: 'COMPLETED' },
    { dayOffset: -4, status: 'CANCELLED' }, { dayOffset: -3, status: 'COMPLETED' },
    { dayOffset: -2, status: 'COMPLETED' }, { dayOffset: -1, status: 'COMPLETED' },
    { dayOffset: 1, status: 'CONFIRMED' }, { dayOffset: 1, status: 'CONFIRMED' },
    { dayOffset: 2, status: 'CONFIRMED' }, { dayOffset: 2, status: 'PENDING' },
    { dayOffset: 4, status: 'CONFIRMED' }, { dayOffset: 5, status: 'CONFIRMED' },
  ...
  for (const [i, entry] of plan.entries()) {
    ...
    const blockMinutes =
      resource.bufferBeforeMinutes + service.durationMinutes +
      Math.max(service.bufferAfterMinutes, resource.bufferAfterMinutes);
    cursors.set(cursorKey, startMinuteLocal + blockMinutes + 15);

    const startsAt = daysAhead(entry.dayOffset, Math.floor(startMinuteLocal / 60), startMinuteLocal % 60);
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        confirmedAt: entry.status !== 'PENDING' && entry.status !== 'CANCELLED' ? daysAgo(Math.abs(entry.dayOffset) + 1) : null,
    ...
    if (entry.status === 'COMPLETED') {
      await prisma.$transaction(async (tx) => {
        const account = await tx.loyaltyAccount.findUniqueOrThrow({ where: { customerId: client.id } });
        const earned = Math.round(service.price * 0.05);
        const balanceAfter = account.balance + earned;
        await tx.loyaltyTransaction.create({
          data: {
            tenantId, accountId: account.id, type: 'EARN', amount: earned, balanceAfter,
    ...
    await prisma.customerTimelineEntry.create({
      data: {
        tenantId, customerId: client.id, type: 'BOOKING_CREATED',
        title: uz(`Bron ${booking.bookingNumber}`, `Запись ${booking.bookingNumber}`),
        amount: service.price, entityType: 'BOOKING', entityId: booking.id,
...
async function seedZeboBeauty() {
  ...
  const branch = await prisma.branch.create({
    data: {
      tenantId, name: 'Zebo Beauty — Yakkasaroy', address: "Yakkasaroy tumani, Bobur ko'chasi 27",
      phone: '998971234567', latitude: 41.2789, longitude: 69.2512, isDefault: true,
      workingHours: WORKDAY_HOURS as Prisma.InputJsonValue,
    },
  });

  const categories = await Promise.all(
    [uz('Tirnoq', 'Ногти'), uz('Soch', 'Волосы'), uz('Yuz parvarishi', 'Уход за лицом')]
      .map((name, i) => prisma.category.create({ data: { tenantId, name, sortOrder: i } })),
  );

  const services = await Promise.all([
    prisma.service.create({ data: { tenantId, categoryId: categories[0]!.id, name: uz('Manikyur', 'Маникюр'), price: 120000, durationMinutes: 60, bufferAfterMinutes: 10, sortOrder: 0 } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[0]!.id, name: uz('Pedikyur', 'Педикюр'), price: 150000, durationMinutes: 75, bufferAfterMinutes: 15, sortOrder: 1 } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[1]!.id, name: uz('Soch turmagi', 'Причёска'), price: 180000, durationMinutes: 90, bufferAfterMinutes: 15, sortOrder: 2 } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[1]!.id, name: uz('Soch bo‘yash', 'Окрашивание волос'), price: 350000, durationMinutes: 150, bufferAfterMinutes: 20, sortOrder: 3 } }),
    prisma.service.create({ data: { tenantId, categoryId: categories[2]!.id, name: uz('Yuz tozalash', 'Чистка лица'), price: 200000, durationMinutes: 60, sortOrder: 4 } }),
  ]);

  const mastersSpec = [
    { firstName: 'Zebo', lastName: 'Nazarova', position: 'Direktor / usta', serviceIdx: [0, 1, 2, 3, 4] },
  ...
  for (const [i, master] of mastersSpec.entries()) {
    const employee = await prisma.employee.create({
      data: {
        tenantId, branchId: branch.id, firstName: master.firstName, lastName: master.lastName,
        position: master.position, phone: `99897123456${i + 1}`, sortOrder: i,
        services: { create: master.serviceIdx.map((si) => ({ tenantId, serviceId: services[si]!.id })) },
      },
    });
    resources.push(await prisma.bookingResource.create({
      data: {
        tenantId, branchId: branch.id, employeeId: employee.id,
        name: `${master.firstName} ${master.lastName}`, kind: 'EMPLOYEE', bufferAfterMinutes: 10,
        schedules: { create: [1, 2, 3, 4, 5, 6].map((weekday) => ({ tenantId, weekday, startTime: '10:00', endTime: '20:00' })) },
      },
    }));
  }
  ...
  await prisma.bookingResource.create({
    data: {
      tenantId, branchId: branch.id, name: 'Kosmetologiya xonasi', kind: 'ROOM',
      schedules: { create: [1, 2, 3, 4, 5, 6].map((weekday) => ({ tenantId, weekday, startTime: '10:00', endTime: '20:00' })) },
    },
  });
  ...
  const clientSpecs = [
    ...
    { firstName: 'Sevara', lastName: 'Tosheva', phone: '998942223344', tgId: 702222222n, language: 'ru' },
    ...
    { firstName: 'Shirin', lastName: 'Valiyeva', phone: '998946667788', tgId: 706666666n, language: 'ru' },
  ];
  ...
  for (const [i, spec] of clientSpecs.entries()) {
    clients.push(await prisma.customer.create({
      data: {
        tenantId, telegramUserId: spec.tgId, telegramUsername: `${spec.firstName.toLowerCase()}_uz`,
        firstName: spec.firstName, lastName: spec.lastName, phone: spec.phone,
        language: spec.language ?? 'uz', source: 'TELEGRAM',
        birthDate: new Date(Date.UTC(1990 + i, i % 12, 5 + i)),
        lastActivityAt: daysAgo(i * 2), createdAt: daysAgo(50 - i * 3),
        loyaltyAccount: { create: { tenantId, balance: 0 } },
      },
    }));
  }
  ...
  await prisma.promoCode.createMany({
    data: [
      { tenantId, code: 'ZEBO20', type: 'PERCENTAGE', value: 20, minOrderTotal: 200000, maxDiscount: 80000, perCustomerLimit: 1, endsAt: daysAhead(60) },
      { tenantId, code: 'DUGONA', type: 'FIXED', value: 50000, minOrderTotal: 150000, usageLimit: 100, endsAt: daysAhead(120) },
    ],
  });
...
async function seedNotificationTemplates(tenantIds: string[]) {
  const templates = [
    { key: 'ORDER_CREATED', body: uz('✅ Buyurtmangiz qabul qilindi!\nRaqam: {{orderNumber}}\nSumma: {{total}}', '✅ Ваш заказ принят!\nНомер: {{orderNumber}}\nСумма: {{total}}') },
    { key: 'ORDER_ACCEPTED', body: uz('👨‍🍳 {{orderNumber}} buyurtmangiz tayyorlanmoqda.', '👨‍🍳 Заказ {{orderNumber}} готовится.') },
    { key: 'ORDER_READY', body: uz('✅ {{orderNumber}} buyurtmangiz tayyor.', '✅ Ваш заказ {{orderNumber}} готов.') },
    { key: 'ORDER_DELIVERING', body: uz('🚗 {{orderNumber}} buyurtmangiz yo‘lda.', '🚗 Заказ {{orderNumber}} в пути.') },
    { key: 'ORDER_COMPLETED', body: uz('🙏 Buyurtmangiz yakunlandi. Rahmat!', '🙏 Заказ завершён. Спасибо!') },
    { key: 'ORDER_CANCELLED', body: uz('❌ {{orderNumber}} buyurtmangiz bekor qilindi.', '❌ Заказ {{orderNumber}} отменён.') },
    { key: 'BOOKING_CREATED', body: uz('✅ Bron tasdiqlandi!\n{{service}}\n{{date}} {{time}}\n{{employee}}', '✅ Запись подтверждена!\n{{service}}\n{{date}} {{time}}\n{{employee}}') },
    { key: 'BOOKING_REMINDER', body: uz('⏰ Eslatma: {{date}} soat {{time}} da {{service}} uchun bronigiz bor.', '⏰ Напоминание: {{date}} в {{time}} у вас запись на {{service}}.') },
    { key: 'BOOKING_CANCELLED', body: uz('❌ {{date}} {{time}} dagi bron bekor qilindi.', '❌ Запись на {{date}} {{time}} отменена.') },
    { key: 'PAYMENT_SUCCESS', body: uz('💳 To‘lov qabul qilindi: {{amount}}', '💳 Оплата получена: {{amount}}') },
    { key: 'LOYALTY_EARNED', body: uz('⭐ Sizga {{amount}} bonus qo‘shildi. Umumiy: {{balance}}', '⭐ Вам начислено {{amount}} бонусов. Всего: {{balance}}') },
  ];

  for (const tenantId of tenantIds) {
    await prisma.notificationTemplate.createMany({
      data: templates.map((t) => ({ tenantId, key: t.key, channel: 'TELEGRAM' as const, body: t.body })),
      skipDuplicates: true,
    });
  }
