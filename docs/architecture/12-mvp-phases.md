...
*Exit:* `pnpm build` green, permission matrix unit-tested.
...
*Exit:* migrations apply cleanly; tenant-guard unit tests pass; seed produces three
...
audit, audit log.
*Exit:* a user registers, creates a business, invites a member; `rbac.spec` and
...
*Exit:* the three acceptance tests in `docs/guides/testing.md` pass end-to-end.
...
*Exit:* `payment-idempotency.spec` and `booking-collision.spec` pass; bot connects and
...
*Exit:* a business can be run end-to-end from the browser.
