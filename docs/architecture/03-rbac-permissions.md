...
|---|---|---|
| *Is this person allowed to do this?* | RBAC — role → permissions | `403 FORBIDDEN` |
| *Has this business turned this feature on?* | Modules — `TenantModule` | `403 MODULE_DISABLED` |
| *Does their plan include it?* | Entitlements — `Plan` → `PlanFeature` | `402 UPGRADE_REQUIRED` |
...
Some roles see only *their* rows. That is not expressible as a permission string, so
