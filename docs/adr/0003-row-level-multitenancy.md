...
- _Database per tenant_: excellent isolation, impossible economics — thousands of
  databases, thousands of connection pools, migrations that take hours.
- _Schema per tenant_: migrations are O(tenants) and Prisma does not model it well.
- _Shared schema_: one migration, one pool, cheap. The cost is that isolation becomes an
