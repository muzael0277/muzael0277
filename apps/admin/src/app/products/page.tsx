...
import {
  Badge, Button, Card, ConfirmDialog, DataTable, EmptyState, Field, Input, Modal,
  MoneyInput, Pagination, Select, Textarea, useToast,
} from '@bizbot/ui';
...
interface Product {
  id: string; name: Record<string, string>; description?: Record<string, string> | null;
  price: number; oldPrice: number | null; sku: string | null;
  isActive: boolean; isFeatured: boolean;
  trackInventory: boolean; stockQuantity: number; lowStockThreshold: number | null;
  effectiveStock: number; isLowStock: boolean; isOutOfStock: boolean;
  categoryId: string | null;
  category?: { id: string; name: Record<string, string> } | null;
}

interface Category { id: string; name: Record<string, string>; children?: Category[] }

const EMPTY_FORM = {
  nameUz: '', nameRu: '', descriptionUz: '',
  price: 0, oldPrice: 0, categoryId: '', sku: '',
  isActive: true, isFeatured: false,
  trackInventory: false, stockQuantity: 0, lowStockThreshold: 0,
};
...
export default function ProductsPage() {
  ...
  const { data, isLoading, mutate } = useSWR<{ data: Product[]; meta: { total: number; totalPages: number } }>(
    tenant ? [`/t/${tenant.id}/catalog/products?${query}`, tenant.id] : null,
    fetcher,
  );
  ...
  async function save() {
    ...
    const body = {
      ...
      lowStockThreshold: form.trackInventory && form.lowStockThreshold > 0 ? form.lowStockThreshold : null,
    };

    setSaving(true);
    try {
      if (editing) {
        await api(`/t/${tenant!.id}/catalog/products/${editing.id}`, { method: 'PATCH', body, tenantId: tenant!.id });
        toast.success('Saqlandi');
      } else {
        await api(`/t/${tenant!.id}/catalog/products`, { method: 'POST', body, tenantId: tenant!.id });
  ...
  async function archive() {
    ...
    setSaving(true);
    try {
      await api(`/t/${tenant!.id}/catalog/products/${deleting.id}`, { method: 'DELETE', tenantId: tenant!.id });
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        action={can('product:write') && <Button onClick={openCreate}>{t(language, 'empty.productsCta')}</Button>}
      />
      ...
      <Card>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              key: 'category', header: 'Kategoriya', secondary: true,
              render: (row) => row.category ? text(row.category.name, language) : '—',
            },
            {
              key: 'price', header: 'Narx', align: 'right',
              render: (row) => (
                <div>
            ...
            {
              key: 'stock', header: 'Qoldiq', align: 'right', secondary: true,
              render: (row) => {
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={save} loading={saving}>{t(language, 'common.save')}</Button>
          </>
        }
      >
        <div className="space-y-4">
          <Field label="Nomi (o‘zbekcha)" required error={errors.name}>
            <Input value={form.nameUz} onChange={(e) => setForm({ ...form, nameUz: e.target.value })} autoFocus />
          </Field>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Eski narx" hint="Chegirmani ko‘rsatish uchun" error={errors.oldPrice}>
              <MoneyInput value={form.oldPrice} onChange={(oldPrice) => setForm({ ...form, oldPrice })} />
            </Field>
          </div>
            ...
            <Field label="Kategoriya">
              <Select value={form.categoryId} onChange={(e) => setForm({ ...form, categoryId: e.target.value })}>
                ...
                {flatCategories.map((category) => (
                  <option key={category.id} value={category.id}>{text(category.name, language)}</option>
                ))}
              </Select>
            </Field>
